...
use eframe::egui;
use vhs_studio_core::{DownscaleMethod, DownscaleSpec, NtscStage, Rotation, ScanlineGrid};
...
pub struct VhsStudioApp {
    // ---- source ----
    pub source: SourceImage,
    pub source_path: Option<PathBuf>,
    /// Applied to the source before anything else. See `vhs_studio_core::rotation`
    ...
    /// `vhs_studio_core::grade`. Edits never touch the frame cache: the grade
    ...
    /// source aspect. See [`VhsStudioApp::export_output_size`].
...
impl VhsStudioApp {
    pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
        let render_state = cc
            .wgpu_render_state
            .as_ref()
            .expect("vhs-studio requires the wgpu backend");
    ...
    pub(crate) fn rotate_decoded(
        ...
        let (pixels, width, height) =
            vhs_studio_core::rotate_rgba(&img.pixels, img.width, img.height, self.rotation);
    ...
    pub fn grade_uniform(&self) -> Option<[f32; vhs_studio_core::GRADE_UNIFORM_LEN]> {
    ...
    fn render_preview(
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("vhs-studio.preview.encode"),
        });
...
pub fn export_output_size(long_edge: u32, chain: (u32, u32), snap: bool, even: bool) -> (u32, u32) {
    ...
    let (mut w, mut h) = if snap {
        vhs_studio_core::ScanlineGrid::snapped_size(chain.0, chain.1, height)
    } else {
