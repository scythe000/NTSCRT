...
use vhs_studio_core::{Easing, Keyframe, Timeline, TimelineEvaluator};
...
impl VhsStudioApp {
    ...
    pub fn timeline_evaluator(&self) -> Option<TimelineEvaluator> {
        ...
        let meta: BTreeMap<String, vhs_studio_core::ShaderMeta> = self
            .shader_param_meta
            .iter()
            .map(|p| {
                (
            ...
            .collect();
        TimelineEvaluator::new(tl, meta, vhs_studio_core::timeline::ntsc_interp_table())
    }
