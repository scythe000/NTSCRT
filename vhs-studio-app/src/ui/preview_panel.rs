...
pub fn show(app: &mut VhsStudioApp, root: &mut egui::Ui, rs: Option<&RenderState>) {
...
fn source_texture(app: &mut VhsStudioApp, ctx: &egui::Context) -> egui::TextureId {
    if !matches!(&app.source_texture, Some((v, _)) if *v == app.source_version) {
        ...
        let handle = ctx.load_texture("vhs-studio.source", image, egui::TextureOptions::NEAREST);
