...
//! ```
//!
//! The grade is this build's own addition (`vhs_studio_core::grade`): a colour
...
use vhs_studio_core::{DownscaleSpec, NtscStage, PixelFormat, ScanlineGrid};
...
pub struct RenderRequest<'a> {
    ...
    pub grade: Option<[f32; vhs_studio_core::GRADE_UNIFORM_LEN]>,
...
impl Pipeline {
    ...
    pub fn copy_last_chain_input(
        ...
        let copy = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("vhs-studio.cached_chain_input"),
    ...
    pub fn render(
            ...
            None => {
                ...
                let tex: wgpu::Texture = match request.downscale {
                    Some(spec) => {
                        ...
                        let upload =
                            Self::obtain(device, &mut self.upload, sw, sh, "vhs-studio.upload", false);
                    ...
                    None => {
                        let tex = Self::obtain(
                            device, &mut self.chain_input, sw, sh, "vhs-studio.chain_input", false,
                        );
        ...
        if !request.shader_enabled {
            ...
            self.downscaler.encode(
                ...
                vhs_studio_core::DownscaleMethod::Nearest,
            );
            return Ok(());
        }
        ...
        if use_supersample {
            let big = Self::obtain(
                device, &mut self.supersample, super_size.0, super_size.1, "vhs-studio.supersample", true,
            );
            ...
            self.downscaler.encode(
                ...
                &big_view, super_size,
                output_view, request.output_size,
                vhs_studio_core::DownscaleMethod::Area,
            );
        } else {
            chain.render(
    ...
    pub fn encode_chain_input_copy(
        ...
        match downscale {
            Some(spec) => {
                let upload = Self::obtain(device, &mut self.upload, sw, sh, "vhs-studio.upload", false);
                Self::write(queue, upload, pixels, sw, sh);
                let up_view = upload.create_view(&Default::default());
