...
//! ```
//!
//! `grade` is this build's colour-grade stage (`vhs_studio_core::grade`), which
...
//! `vhs_studio_core::timeline` for the model and how it interpolates.
...
mod tests {
    ...
    fn sample() -> AppPreset {
        AppPreset {
            ...
            timeline: Some(vhs_studio_core::Timeline {
                ...
                fps: 30.0,
                enabled: false,
                keys: vec![vhs_studio_core::Keyframe {
                    t: 0.0,
                    easing: vhs_studio_core::Easing::Linear,
    ...
    fn round_trips_through_disk() {
        let path = std::env::temp_dir().join("vhs-studio-preset-test.json");
        let p = sample();
        p.save(&path).unwrap();
    ...
    fn keyframes_survive_a_load_and_save_cycle() {
        ...
        let path = std::env::temp_dir().join("vhs-studio-preset-keys.json");
    ...
    fn a_preset_without_keyframes_reports_none() {
        let mut p = sample();
        p.timeline = Some(vhs_studio_core::Timeline::default());
    ...
    fn a_future_version_is_refused_rather_than_misread() {
        let path = std::env::temp_dir().join("vhs-studio-preset-v99.json");
