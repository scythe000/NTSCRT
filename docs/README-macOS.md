...
> This is the README of the original **NTSCRT** macOS app by Finn McKenty,
> kept here because that app still lives in this repository (`Sources/`,
> `Package.swift`). The repository's front page is now
> [VHS-Studio](../README.md), the Windows app that grew out of it. For the
> upstream project itself see
> [finnmckenty/NTSCRT](https://github.com/finnmckenty/NTSCRT).

![NTSCRT — the full NTSC + CRT pipeline on the left of the compare split, untouched source on the right, with a keyframed animation on the timeline below](header.webp)
...
Grab the DMG from [**Releases**](https://github.com/finnmckenty/NTSCRT/releases/latest), open it, and drag **NTSCRT** to Applications.

**Requirements:** macOS 14 or later. The app is a universal binary (Apple Silicon + Intel).

> **Intel note:** I build and test NTSCRT on Apple Silicon and haven't personally tested the Intel build. Intel support exists thanks to a contributed fix ([finnmckenty/NTSCRT#1](https://github.com/finnmckenty/NTSCRT/pull/1)) verified by its author on an Intel iMac Pro — if something misbehaves on your Intel Mac, please open an issue.
